...
export function EditStaffV2() {
    ...
    useEffect(() => {
        if (!id) return;
        setPageLoading(true);
        fetch(`${getApiUrl()}/api/v2/staff/${id}`, { headers: getAuthHeaders() })
            .then(res => res.json())
            .then(data => {
                if (!data?.data) throw new Error(data?.message || 'Không tìm thấy nhân viên');
                ...
                setWorkSchedule((staff.workSchedule || []).map((shift: WorkShift) => ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(data.message || 'Cập nhật nhân viên thất bại');
            setSuccess(true);
        ...
        <AdminLayout>
            <div className="max-w-4xl mx-auto space-y-6 pb-12">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">Sửa Nhân viên V2</h1>
                        <p className="text-slate-500 text-sm mt-1">Cập nhật thông tin, vai trò, phân quyền và lịch làm việc</p>
                    </div>
                    <button
                ...
                {success && (
                    <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 p-4 rounded-2xl flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold">Cập nhật nhân viên thành công! Đang chuyển trang...</span>
                    </div>
                )}
                {error && (
                ...
                <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-slate-700 uppercase mb-2">Tài Khoản</label>
                            <input
                                type="text"
                                disabled
                                value={form.account}
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-700 uppercase mb-2">Mật Khẩu Mới</label>
                            <input
                                type="password"
                                value={form.password}
