  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      const validMembership =
        memberships.find((m) => m.valid) || memberships[0];
      if (validMembership) {
      ...
      <div className="max-w-7xl mx-auto space-y-6 pb-12">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Tạo lịch tập cho khách hàng với PT, tự kiểm tra trùng khung giờ
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              <h2 className="text-sm font-bold text-slate-700 uppercase flex items-center gap-2 mb-4">
                <Search className="w-4 h-4 text-indigo-500" /> Bước 1 · Tra cứu
                khách hàng
              </h2>
              <div className="flex flex-col md:flex-row gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                  {phone && (
                    <button
                      onClick={() => setPhone("")}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="font-bold text-emerald-800">
                        {lookedUp.customerName}
                      </p>
                      <p className="text-xs text-emerald-700">
                        {lookedUp.customerPhone}
                        {lookedUp.customerEmail
                          ? ` · ${lookedUp.customerEmail}`
                          : ""}
                      </p>
                        ...
                        <div className="mt-2">
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              const pkg = lookedUp.memberships.find(
                    ...
                    <button
                      ...
                      className="p-1 text-emerald-500 hover:bg-emerald-100 rounded-lg"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              <h2 className="text-sm font-bold text-slate-700 uppercase flex items-center gap-2 mb-4">
                <Dumbbell className="w-4 h-4 text-indigo-500" /> Bước 2 · Thông
                tin buổi tập
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase mb-1 flex items-center gap-1">
                    <Users className="w-3.5 h-3.5" /> Huấn luyện viên{" "}
                    {loadingTrainers && (
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
              <h2 className="text-sm font-bold text-slate-700 uppercase flex items-center gap-2 mb-4">
                <Clock className="w-4 h-4 text-indigo-500" /> Khung giờ rảnh của
                PT
              </h2>
              {!trainerId ? (
                <p className="text-sm text-slate-400">
                  Chọn PT để xem lịch rảnh.
                </p>
              ) : checkingAvailability ? (
                <div className="py-8 text-center text-slate-400">
                  <Loader2 className="w-5 h-5 animate-spin inline" /> Đang
                  tải...
                </div>
              ) : availabilityError ? (
                ...
                availability.working ? (
                  <>
                    <div className="text-xs text-slate-500 mb-3">
                      Ca làm việc: {availability.shift?.startTime} -{" "}
                      {availability.shift?.endTime} ·{" "}
                      {availability.availableSlots.length} khung trống ·{" "}
                      {availability.busyBookings.length} buổi đã đặt
                    </div>
                  ...
                  <p className="text-sm text-amber-600 flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4" /> PT không làm việc vào
                    ngày này.
                  </p>
                )
              ) : (
                <p className="text-sm text-slate-400">
                  Chọn ngày để tải lịch rảnh.
                </p>
              )}
            </div>
