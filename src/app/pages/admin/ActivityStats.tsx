    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center py-20">
                ...
                <span className="ml-3 text-slate-500 text-sm">Đang tải dữ liệu hoạt động...</span>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <p className="text-slate-500 text-sm">Dữ liệu liên kết trực tiếp từ Database thời gian thực</p>
                </div>
                <button
                    ...
                    {bookingChartData.map((item, idx) => {
                        ...
                        return (
                            <div key={idx} className="flex-1 max-w-[140px] flex flex-col items-center group relative">
                                ...
                                >
                                    <span className="text-white text-xs font-black drop-shadow">{item.count}</span>
                                </div>
                                <span className="text-xs text-slate-600 font-black mt-3 text-center">{item.label}</span>
                            </div>
                        );
                    })}
                </div>
                                ...
                                return (
                                    <div key={idx} className="space-y-1">
                                        ...
                                        <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                                            <div style={{ width: `${percent}%` }} className={`h-full ${colors[idx % colors.length]} transition-all duration-700`}></div>
                                        </div>
                                    </div>
                                );
                            })}
                            ...
                            {checkInOfWeek.map((item, idx) => {
                                ...
                                return (
                                    <div key={idx} className="flex-1 flex flex-col items-center group relative">
                                        ...
                                        <div style={{ height: `${heightVal}px` }} className="w-full bg-purple-500 hover:bg-purple-600 rounded-t-md transition-all duration-500"></div>
                                    ...
                                    return (
                                        <div key={idx} className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100 space-y-2">
                                            ...
                                            <div className="w-full h-2.5 bg-slate-200 rounded-full overflow-hidden">
                                                <div style={{ width: `${percentOfMax}%` }} className={`h-full transition-all duration-700 ${idx === 0 ? 'bg-amber-500' : 'bg-indigo-500'}`}></div>
                                            </div>
                                        </div>
                                    );
                                });
