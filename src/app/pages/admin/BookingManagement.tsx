...
export function BookingManagement() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const res = await fetch(`${getApiUrl()}/api/bookings?${params.toString()}`, { headers });
      if (res.ok) {
  ...
  const handleReject = async () => {
    ...
    try {
      const headers = getAuthHeaders();
      const res = await fetch(`${getApiUrl()}/api/bookings/${rejectDialog.bookingId}/reject`, {
        method: 'PUT',
  ...
  return (
    <AdminLayout>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex-1 min-w-[300px]">
              <div className="relative">
                ...
                <input type="text" placeholder="Tìm theo tên học viên, HLV..."
            ...
            <div className="flex gap-2">
              {['all', 'pending', 'confirmed', 'rejected'].map((status) => (
                <button key={status} onClick={() => setStatusFilter(status)}
                  className={`px-4 py-2 rounded-lg font-semibold transition-all ${statusFilter === status ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div className="flex items-center gap-2 text-slate-600">
                          ...
                          <span>Ngày: <span className="font-semibold">{new Date(booking.date).toLocaleDateString('vi-VN')}</span></span>
                        </div>
                        <div className="flex items-center gap-2 text-slate-600">
                        ...
                        <div className="flex items-center gap-2 text-slate-600">
                          <span>Bộ môn: <span className="font-semibold">{booking.disciplineId?.name || booking.disciplineName || 'N/A'}</span></span>
                        </div>
                      </div>
                    ...
                    {booking.status === 'pending' && (
                      <div className="flex gap-2 ml-4">
                        <Button variant="contained" startIcon={<CheckCircle />} onClick={() => handleConfirm(booking._id)}
                          sx={{ bgcolor: '#10b981', '&:hover': { bgcolor: '#059669' }, textTransform: 'none' }}>
                          Xác nhận
                        </Button>
                        <Button variant="outlined" color="error" startIcon={<XCircle />}
      ...
      <Dialog open={rejectDialog.open} onClose={() => setRejectDialog({ open: false, bookingId: '' })} maxWidth="sm" fullWidth>
        <DialogTitle>Từ chối lịch đặt</DialogTitle>
        <DialogContent>
          <TextField fullWidth multiline rows={3} label="Lý do từ chối *"
            value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)}
            placeholder="Nhập lý do từ chối lịch đặt..." sx={{ mt: 1 }} />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => { setRejectDialog({ open: false, bookingId: '' }); setRejectionReason(''); }}
            sx={{ textTransform: 'none' }}>Hủy</Button>
          <Button variant="contained" color="error" onClick={handleReject} sx={{ textTransform: 'none' }}>Từ chối</Button>
        </DialogActions>
      </Dialog>
    </AdminLayout>
  );
