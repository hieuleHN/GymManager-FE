  ...
  const fetchHistory = async (p = page) => {
    try {
      const res = await fetch(
        `${getApiUrl()}/api/salary/history?page=${p}&limit=15`,
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">
            Lịch sử trả lương nhân viên
          </h1>
          <p className="text-slate-600">
            Xem lại lịch sử trả lương cho nhân viên
          </p>
        </div>
                ...
                {history.map((record, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-slate-900">
                      {record.staffId?.fullName || "Đã xóa"}
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm text-indigo-600 font-bold">
                      {record.totalSalary.toLocaleString("vi-VN")}đ
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">
                      {new Date(record.paidAt).toLocaleDateString("vi-VN")}
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">
                      {record.paidBy?.fullName || "Hệ thống"}
                    </td>
                  </tr>
                ))}
                {history.length === 0 && (
