...
import { CheckCircle, Calendar, Clock, User, Check, X, Info } from 'lucide-react';
            ...
            <div key={request.id} className="p-6 hover:bg-slate-50 transition-colors">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-lg font-bold text-slate-900">{request.memberName}</h3>
                  ...
                  <div className="grid md:grid-cols-2 gap-4 mb-3">
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      ...
                      Giờ: <span className="font-semibold text-slate-900">{request.requestedTime}</span>
                    </div>
                    <div className="text-sm text-slate-600">
                      Dịch vụ: <span className="font-semibold text-slate-900">{request.service}</span>
                    </div>
                    <div className="text-sm text-slate-600">
                  ...
                  {request.note && (
                    <div className="p-3 bg-blue-50 rounded-lg border border-blue-200 text-sm">
                      ...
                      <span className="text-blue-800 ml-2">{request.note}</span>
                    </div>
                  )}
                </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full">
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-6">
                ...
                />
                <div>
                  <h4 className="text-xl font-bold text-slate-900">{selectedRequest.memberName}</h4>
                  <p className="text-slate-600">📞 {selectedRequest.memberPhone}</p>
                  <p className="text-slate-600">✉️ {selectedRequest.memberEmail}</p>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                ...
                <div className="p-4 bg-slate-50 rounded-lg">
                  ...
                  <p className="text-lg font-semibold text-slate-900 mt-1">{selectedRequest.requestedTime}</p>
                </div>

                <div className="p-4 bg-slate-50 rounded-lg">
                  <label className="text-sm font-medium text-slate-700">Dịch vụ</label>
                  <p className="text-lg font-semibold text-slate-900 mt-1">{selectedRequest.service}</p>
                </div>
